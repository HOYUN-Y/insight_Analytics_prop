...
## [0.6.4] — 2026-06-08 · Map Studio — 베이스 지도(스타일) 전환

### Added — `js/mapStudio.jsx`
- **베이스 지도 선택** 드롭다운: 기본 지도(streets-v2) / 심플(basic-v2) / 연한·오버레이용(dataviz-light) / 위성(hybrid)
- `map.setStyle()` 교체 후 `styleEpoch` 증가 → 반경·철도·도로·포인트·색상·옵션 레이어 **자동 재적용**
- 반경 동심원 line 레이어를 별도 effect로 분리해 스타일 교체 시 재생성 (라벨/핀 마커는 유지)
- 검증: dataviz-light 전환 시 전체 오버레이 유지 확인

### Note — 국토부 도시철도 데이터 평가
- `docs/reference/data reference/국토교통부_도시철도 전체노선_20251211.csv` (EUC-KR, 1,103역): 권역·운영기관·노선명·순번·역명
- **좌표 없음** → 지도 레이어로 직접 사용 불가. 향후 용도: 노선/역 공식 명칭·순서 참조, 보고서 광역입지 표, 좌표 조인용. 지도 지오메트리는 OSM 유지.
